...
abstract class AbstractOpenAddressingSetTest {
    ...
    protected fun doRemoveTest() {
        ...
        for (iteration in 1..300) {
            val bitsNumber = random2.nextInt(2) + 5
            val openAddressingSet = create<Int>(bitsNumber)
            for (i in 1..60) {
                val firstInt = random2.nextInt(23)
    ...
    protected fun doIteratorTest() {
        ...
        for (iteration in 1..200) {
            val controlSet = mutableSetOf<String>()
            for (i in 1..15) {
                val string = random2.nextString("absfgesq12344278", 1, 15)
                controlSet.add(string)
            }
            println("Control set: $controlSet")
            val openAddressingSet = create<String>(random2.nextInt(2) + 6)
            assertFalse(
                openAddressingSet.iterator().hasNext(),
                "Iterator of an empty set should not have any next elements."
            )
    ...
    protected fun doIteratorRemoveTest() {
        ...
        for (iteration in 1..200) {
            val controlSet = mutableSetOf<String>()
            val removeIndex = random2.nextInt(12) + 4
            var toRemove = ""
            for (i in 1..15) {
                val string = random2.nextString("abawacgd1253718", 1, 15)
                controlSet.add(string)
                if (i == removeIndex) {
                    toRemove = string
                }
            ...
            println("Initial set: $controlSet")
            val openAddressingSet = create<String>(random2.nextInt(6) + 4)
            for (element in controlSet) {
                openAddressingSet += element
            }
            controlSet.remove(toRemove)
